...
API_KEY = 'sk_Lxk4Kr65RzupAJ9xRt6Yg'
...
def sarsa(env, n_episodes, discount_factor=0.99, alpha=0.7, epsilon=0.1):
    ...
    pol = epsilon_greedy_policy(Q, epsilon, env.action_space.n)
    ...
    for i_episode in range(n_episodes):
        ...
        estado = env.reset()
        act_probs = policy(estado)
        act = np.random.choice(np.arange(len(act_probs)), p=act_probs)
        ...
        for t in itertools.count():
            ...
            next_act_probs = policy(next_state)
            next_act = np.random.choice(np.arange(len(next_act_probs)), p=next_act_probs)
            
           
            
         
            td_target = reward + discount_factor * Q[next_state][next_action]
            td_delta = td_target - Q[state][act]
            Q[state][act] += alpha * td_delta
